...
int main()
{
  ...
  NodeId zero = sim.add_node(1, nullptr);
  ...
  sim.get_node(zero).set_strategy(std::make_unique<CongestionAwareStrategy>(zero, sim, 1, 0, 0, 1));
  sim.get_node(one).set_strategy(std::make_unique<CongestionAwareStrategy>(one, sim, 1, 0, 0, 1));
  // sim.get_node(two).set_strategy(std::make_unique<CongestionAwareStrategy>(two, sim, 1, 0, 0, 1));
  // sim.get_node(three).set_strategy(std::make_unique<CongestionAwareStrategy>(three, sim, 1, 0, 0, 1));
  sim.get_node(four).set_strategy(std::make_unique<CongestionAwareStrategy>(four, sim, 1, 0, 0, 1));
