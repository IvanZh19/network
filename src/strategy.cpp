...
NodeId CongestionAwareStrategy::choose_next_hop(NodeId self_id, Packet& p, Simulation& sim) const
{
  ...
  // purely local and greedy
  for (const Link& l : sim.get_links(self_id))
  {
    NodeId n = l.to();

    SimTime estimate = (this->congestion_factor * (l.next_free_time() - sim.now()))
                      + l.propagation_delay() + p.packet_size / l.bandwidth();
...
std::unique_ptr<Strategy> make_strategy(StrategyType type, NodeId self_id, Simulation& sim, const StrategyParams& params)
{
  switch (type)
  {
    case StrategyType::Random:
      return std::make_unique<RandomNeighborStrategy>(self_id, sim);
    case StrategyType::ShortestPath:
      return std::make_unique<ShortestPathStrategy>(
          self_id, sim, params.pd_factor, params.bw_factor, params.fixed);
    case StrategyType::CongestionAware:
      return std::make_unique<CongestionAwareStrategy>(
          self_id, sim, params.congestion_factor);
  }
